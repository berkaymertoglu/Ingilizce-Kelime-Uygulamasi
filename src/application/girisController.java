...
public class girisController {
    ...
    void txt_sifremiunuttum_click(ActionEvent event) {
        ...
        if (result.isPresent()) {
            String to = result.get(); // Alıcının e-posta adresi
            String subject = "Şifre Hatırlatma"; // E-posta konusu

            // Şifreyi çözmek için veritabanından sorgu yap
            ...
            try {
                ...
                if (resultSet.next()) {
                    String encryptedPassword = resultSet.getString("sifre"); // Veritabanından şifreyi al
                    String decryptedPassword = SifrelemeUtils.decrypt(encryptedPassword); // Şifreyi çöz
                    

                    String body = "Şifreniz: " + decryptedPassword; // E-posta içeriği
